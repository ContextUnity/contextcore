...
if TYPE_CHECKING:
    from contextcore.manifest.models import ContextUnityProject
...
# Canonical: CONTEXTROUTER_GRPC_URL, CONTEXTBRAIN_GRPC_URL, etc. (uniform *_GRPC_URL suffix)
_SERVICE_ENV: dict[str, tuple[str, str]] = {
    "router": ("CONTEXTROUTER_GRPC_URL", "localhost:50051"),
    "brain": ("CONTEXTBRAIN_GRPC_URL", "localhost:50052"),
    "worker": ("CONTEXTWORKER_GRPC_URL", "localhost:50053"),
    "shield": ("CONTEXTSHIELD_GRPC_URL", "localhost:50054"),
    "zero": ("CONTEXTZERO_GRPC_URL", "localhost:50055"),
    "commerce": ("CONTEXTCOMMERCE_GRPC_URL", "localhost:50056"),
}
...
class ProjectBootstrapConfig(BaseModel):
    ...
    router_url: str = Field(default="", description="ContextRouter gRPC URL")
    brain_url: str = Field(default="", description="ContextBrain gRPC URL")
    worker_url: str = Field(default="", description="ContextWorker gRPC URL")
    shield_url: str = Field(default="", description="ContextShield gRPC URL")
    zero_url: str = Field(default="", description="ContextZero gRPC URL")
    commerce_url: str = Field(default="", description="ContextCommerce gRPC URL")
    ...
    def from_env(cls, **overrides: Any) -> ProjectBootstrapConfig:
        ...
        # Manifest path
        manifest = os.environ.get("CONTEXTUNITY_MANIFEST_PATH", "")
        if manifest:
