...
def validate_safe_url(url: object, allow_local: bool = False) -> str:
    ...
    try:
        ...
        raise SecurityError(f"Failed to parse URL: {e}")

    # 1. Block dangerous schemes immediately
    if parsed.scheme.lower() not in ("http", "https"):
        raise SecurityError(f"Unsafe URL scheme '{parsed.scheme}'. Only http and https are allowed.")
    ...
    if not allow_local:
        ...
        if hostname.lower() in forbidden_hosts:
            raise SecurityError(f"URL targeting protected/local hostname '{hostname}' is not allowed.")
