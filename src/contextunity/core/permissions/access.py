...
def has_tool_access(permissions: tuple[str, ...] | list[str], tool_name: str) -> bool:
    ...
    Examples:
        >>> token_perms = ["tool:brain_search", "tool:shield_scan"]
        >>> has_tool_access(token_perms, "brain_search")
        True
        >>> has_tool_access(token_perms, "sql_execute")
        False
    """
...
def has_registration_access(
    ...
    Checks in order:
      1. ``tools:register:{project_id}`` (project-specific)
      2. ``admin:all`` (superadmin)

    The generic ``tools:register`` string is not sufficient for
    RegisterManifest. Registration is project-scoped to prevent cross-project
    takeover.
    ...
    Examples:
        >>> has_registration_access(("tools:register:nszu",), "nszu")
        True
        >>> has_registration_access(("tools:register:nszu",), "acme")
        False
    """
    perm_set = set(permissions)
    return Permissions.ADMIN_ALL in perm_set or f"tools:register:{project_id}" in perm_set


def has_introspection_access(
    permissions: tuple[str, ...] | list[str],
    project_id: str,
) -> bool:
    """Check if a permission set grants manifest introspection for *project_id*.

    ``project_id`` is the registration identity — not an entry in
    ``allowed_tenants``. Tenant scope governs execution; introspection is
    project-scoped via explicit permissions.

    Checks in order:

      1. ``admin:all``
      2. ``router:introspect:{project_id}``
      3. ``tools:register:{project_id}`` (operators who registered the bundle)

    Bare ``router:introspect`` satisfies the RPC interceptor only; listing or
    reading a project still requires a project-specific grant above.
    """
    perm_set = set(permissions)
    if Permissions.ADMIN_ALL in perm_set:
        return True
    if f"router:introspect:{project_id}" in perm_set:
        return True
    return f"tools:register:{project_id}" in perm_set
...
def has_graph_access(permissions: tuple[str, ...] | list[str], graph_name: str) -> bool:
    ...
    Examples:
        >>> token_perms = ["graph:rag_retrieval"]
        >>> has_graph_access(token_perms, "rag_retrieval")
        True
        >>> has_graph_access(token_perms, "commerce_search")
        False
    """
    perm_set = set(permissions)
    return (
...
def extract_tool_names(permissions: tuple[str, ...] | list[str]) -> frozenset[str]:
    ...
    Returns:
        frozenset[str]: Frozenset of tool names (e.g., ``frozenset({"brain_search", "shield_scan"})``).
            Contains ``"*"`` if wildcard is granted.

    Examples:
        >>> perms = ("tool:brain_search", "tool:shield_scan", "brain:read")
        >>> extract_tool_names(perms)
        frozenset({'brain_search', 'shield_scan'})
...
def has_tool_scope_access(
    ...
    Returns:
        bool: True if the token authorizes the operation, False otherwise.

    Examples:
        >>> perms = ("tool:sql:read",)
        >>> has_tool_scope_access(perms, "sql", ToolScope.READ)
        True
        >>> has_tool_scope_access(perms, "sql", ToolScope.WRITE)
        False
        >>> perms = ("tool:sql:write",)
        >>> has_tool_scope_access(perms, "sql", ToolScope.READ)
        True
    """
    perm_set = set(permissions)
...
def check_tool_scope(
    ...
    Returns:
        str: ToolRisk string (``"safe"``, ``"confirm"``, or ``"deny"``).

    Examples:
        >>> check_tool_scope(perms, "sql", ToolScope.READ)
        'safe'
        >>> check_tool_scope(perms, "sql", ToolScope.WRITE)
        'deny'
    """
